...
static int _dump_emummc_raw_part(emmc_tool_gui_t *gui, int active_part, int part_idx, u32 sd_part_off, emmc_part_t *part, u32 resized_count)
{
	...
	s_printf(gui->txt_buf, "#96FF00 Carp. base:#\n%s\n#96FF00 Offset particion:# #FF8000 0x%08X#",
	...
	if (resized_count)
	{
		...
		if (mkfs_error)
		{
			s_printf(gui->txt_buf, "#FF0000 Error (%d)!#\nIntentalo de nuevo...\n", mkfs_error);
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		...
		s_printf(gui->txt_buf, "Escribiendo nuevo GPT... ");
		lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
		manual_system_maintenance(true);
...
static int _emummc_raw_derive_bis_keys(emmc_tool_gui_t *gui, u32 resized_count)
{
	...
	if (error)
	{
		...
		lv_mbox_set_text(mbox, "#C7EA46 Generacion de BIS Keys#");
