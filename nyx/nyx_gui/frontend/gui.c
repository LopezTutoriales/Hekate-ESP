...
static void _save_fb_to_bmp()
{
	...
	lv_mbox_set_text(mbox, SYMBOL_CAMERA"  #FFDD00 Guardando captura...#");
	...
	if (!res)
		lv_mbox_set_text(mbox, SYMBOL_CAMERA"  #96FF00 Captura guardada!#");
	else
		lv_mbox_set_text(mbox, SYMBOL_WARNING"  #FFDD00 Captura fallo!#");
	manual_system_maintenance(true);
...
bool nyx_emmc_check_battery_enough()
{
	...
	if (batt_volt && batt_volt < 3650)
	{
		...
		lv_mbox_set_text(mbox,
			"#FF8000 Comprobando Bateria#\n\n"
			"#FFDD00 Bateria insuficiente para hacer#\n"
			"#FFDD00 la operacion seleccionada!#\n\n"
			"Cargala al menos a #C7EA46 3650 mV#, y reintentalo!");
...
lv_obj_t *nyx_create_standard_window(const char *win_title)
{
	...
	close_btn = lv_win_add_btn(win, NULL, SYMBOL_CLOSE" Cerrar", lv_win_close_action_custom);
...
lv_obj_t *nyx_create_window_custom_close_btn(const char *win_title, lv_action_t rel_action)
{
	...
	close_btn = lv_win_add_btn(win, NULL, SYMBOL_CLOSE" Cerrar", rel_action);
...
static void _check_sd_card_removed(void *params)
{
	...
	if (!do_reload && sd_get_card_removed())
	{
		...
		static const char * mbox_btn_map[] = { "\221Reiniciar (RCM)", "\221Apagar", "\221No recargar", "" };
		static const char * mbox_btn_map_rcm_patched[] = { "\221Reiniciar", "\221Apagar", "\221No recargar", "" };
		lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
		lv_mbox_set_recolor_text(mbox, true);
...
static lv_res_t _create_mbox_reload(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\221Recargar", "\221Cancelar", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES * 4 / 10);

	lv_mbox_set_text(mbox, "#FF8000 De verdad quieres#\n#FF8000 recargar hekate y Nyx?#\n\n"
		"Esto comprueba\n#96FF00 bootloader/update.bin#\npara actual. de hekate");
...
static lv_res_t _create_mbox_reboot(lv_obj_t *btn)
{
	...
	lv_mbox_set_text(mbox, "#FF8000 Elije como reiniciar:#");
...
static lv_res_t _create_mbox_poweroff(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\221Apagar", "\221Cancelar", "" };
	lv_obj_t *mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES * 4 / 10);
...
static void _create_tab_about(lv_theme_t * th, lv_obj_t * parent)
{
	...
	char version[32];
	s_printf(version, "Nyx v%d.%d.%d ESP", NYX_VER_MJ, NYX_VER_MN, NYX_VER_HF);
...
static lv_res_t _create_mbox_payloads(lv_obj_t *btn)
{
	...
	lv_mbox_set_text(mbox, "Selecciona payload para lanzar:");
	...
	if (!sd_mount())
	{
		lv_mbox_set_text(mbox, "#FFDD00 Error al iniciar la SD!#");
...
static lv_obj_t *create_window_launch(const char *win_title)
{
	...
	close_btn = lv_win_add_btn(win, NULL, SYMBOL_CLOSE" Cerrar", _win_launch_close_action);
...
static lv_res_t _create_window_home_launch(lv_obj_t *btn)
{
	...
	if (btn)
	{
		if (strcmp(lv_label_get_text(lv_obj_get_child(btn, NULL)) + 8,"Lanzar#"))
			more_cfg = true;
	}
	else
	{
	...
	if (!btn)
		win = create_window_launch(SYMBOL_GPS" Hekate - Lanzar");
	else if (!more_cfg)
		win = create_window_launch(SYMBOL_GPS" Lanzar");
	else
		win = create_window_launch(SYMBOL_GPS" Mas Ajustes");
	...
	if (no_boot_entries)
	{
		...
		if (!more_cfg)
		{
			lv_label_set_static_text(label_error,
				"#FFDD00 Sin entradas de arranque...#\n"
				"Mira que #96FF00 bootloader/hekate_ipl.ini# tenga entradas de arranque\n"
				"o usa el boton #C7EA46 Mas Ajustes# para mas opciones de arranque.");
		}
		else
		{
			lv_label_set_static_text(label_error,
				"#FFDD00 Sin archivo .ini o arranques encontrados...#\n"
				"Mira que un archivo .ini existe en #96FF00 bootloader/ini/#\n"
				"y que contiene al menos una entrada de arranque.");
		}
...
static void _create_tab_home(lv_theme_t *th, lv_obj_t *parent)
{
	...
	s_printf(btn_colored_text, "%s%s", text_color, " Hekate ESP#");
	...
	lv_label_set_static_text(label_tagline, "EL BOOTLOADER TODO EN UNO QUE NECESITAS\nTraducido por LOPEZ TUTORIALES");
	...
	s_printf(btn_colored_text, "%s%s", text_color, " Lanzar#");
	...
	s_printf(btn_colored_text, "%s%s", text_color, " Mas Ajustes#");
	lv_label_set_text(label_btn2, btn_colored_text);
	...
	_create_text_button(th, NULL, btn_nyx_options, SYMBOL_SETTINGS" Ajustes de Nyx", NULL);
	...
	s_printf(btn_colored_text, "%s%s", text_color, " EmuNAND#");
	lv_label_set_text(label_btn2, btn_colored_text);
	lv_obj_align(label_btn2, NULL, LV_ALIGN_IN_TOP_MID, 0, 174);
	...
	_create_text_button(th, NULL, btn_power_off, SYMBOL_POWER" Apagar", _create_mbox_poweroff);
	...
	_create_text_button(th, NULL, btn_reload, SYMBOL_REFRESH" Recargar", _create_mbox_reload);
...
static lv_res_t _save_options_action(lv_obj_t *btn)
{
	...
	if (res)
		lv_mbox_set_text(mbox, "#FF8000 Configuracion de hekate#\n\n#96FF00 Configuracion guardada en la SD!#");
	else
...
static void _create_status_bar(lv_theme_t * th)
{
	...
	lv_label_set_static_text(lbl_mid, "Guardar Ajustes");
...
void nyx_check_ini_changes()
{
	if (nyx_options_get_ini_changes_made())
	{
		...
		lv_mbox_set_text(mbox,
			"#FF8000 Configuracion Principal#\n\n"
...
static void _nyx_main_menu(lv_theme_t * th)
{
	...
	char version[32];
	s_printf(version, "hekate v%d.%d.%d ESP", nyx_str->version & 0xFF, (nyx_str->version >> 8) & 0xFF, (nyx_str->version >> 16) & 0xFF);
	lv_obj_t *tab_about = lv_tabview_add_tab(tv, version);

	lv_obj_t *tab_home = lv_tabview_add_tab(tv, SYMBOL_HOME" INICIO");

	lv_obj_t *tab_tools = lv_tabview_add_tab(tv, SYMBOL_TOOLS" HERRAMIENT.");
	lv_page_set_style(tab_tools, LV_PAGE_STYLE_BG, &no_padding);
	lv_page_set_style(tab_tools, LV_PAGE_STYLE_SCRL, &no_padding);

	lv_obj_t *tab_info = lv_tabview_add_tab(tv, SYMBOL_INFO" INFO CONSOLA");
	lv_page_set_style(tab_info, LV_PAGE_STYLE_BG, &no_padding);
	lv_page_set_style(tab_info, LV_PAGE_STYLE_SCRL, &no_padding);

	lv_obj_t *tab_options = lv_tabview_add_tab(tv, SYMBOL_SETTINGS" OPCIONES");
