...
static void _create_window_emummc()
{
	...
	lv_obj_t *win;
	if (!mbr_ctx.part_idx)
		win = nyx_create_window_custom_close_btn(SYMBOL_DRIVE"  Crear Archivo SD EmuNAND", _action_emummc_window_close);
	else
		win = nyx_create_window_custom_close_btn(SYMBOL_DRIVE"  Crear Particion SD EmuNAND", _action_emummc_window_close);
...
static void _create_mbox_emummc_raw()
{
	...
	static const char *mbox_btn_format[] = { "\222Continuar", "\222Cancelar", "" };
	static char *mbox_btn_parts[] = { "\262Part 1", "\262Part 2", "\262Part 3", "\222Cancelar", "" };
	...
	s_printf(txt_buf + strlen(txt_buf),
		...
		"#%s Part 3: Tipo: %02x, Inicio: %08x, Tam: %08x#",
	...
	if (mbr_ctx.resized_cnt[0] || mbr_ctx.resized_cnt[1] || mbr_ctx.resized_cnt[2])
		strcat(txt_buf, "\n\n#FFDD00 Nota:# Las amarillas tienen part. USER redimensio.");
...
static lv_res_t _create_mbox_emummc_create(lv_obj_t *btn)
{
	...
	static const char * mbox_btn_map[] = { "\222Archivo SD", "\222Particion SD", "\222Cancelar", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox,
		"Bienvenido a la herramienta para crear una #C7EA46 EmuNAND#!\n\n"
		"Elije el tipo de EmuNAND que quieres crear.\n"
		"#FF8000 Archivo SD# se guarda como archivos en la SD.\n"
		"#FF8000 Particion SD# se guarda como imagen raw en una particion disponible.");
...
static void _create_emummc_migrated_mbox()
{
	...
	lv_mbox_set_text(mbox,
		"#FF8000 Configuracion de EmuNAND#\n\n"
		"#96FF00 Configuracion de EmuNAND#\n#96FF00 guardada en la SD!#");
...
static lv_res_t _create_emummc_migrate_action(lv_obj_t * btns, const char * txt)
{
	...
	static const char *mbox_btn_map1[] = { "\222Archivo SD", "\222Particion SD", "\222Cancelar", "" };
...
static lv_res_t _create_mbox_emummc_migrate(lv_obj_t *btn)
{
	...
	static char *mbox_btn_map[] = { "\262Backup", "\262Arreglar RAW", "\262EmuNAND", "\222Cancelar", "" };
	lv_obj_t * mbox = lv_mbox_create(dark_bg, NULL);
	lv_mbox_set_recolor_text(mbox, true);
	lv_obj_set_width(mbox, LV_HOR_RES / 9 * 6);

	lv_mbox_set_text(mbox,
		"Bienvenido a la herramienta de migracion de EmuNAND!\n\n"
		"Elije del tipo de migracion que quieres hacer.\n"
		"Lo que no se encuentre tendra el boton deshabilitado.");
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_SETTINGS"  Cambiar EmuNAND");
	lv_win_add_btn(win, NULL, SYMBOL_POWER"  Desactivar", _save_disable_emummc_cfg_action);
...
out0:;
	...
	lv_label_set_static_text(label_txt, "Particiones SD Raw");
	...
	for (u32 raw_btn_idx = 0; raw_btn_idx < 3; raw_btn_idx++)
	{
		...
		s_printf(txt_buf, "Inicio Sector: 0x%08X\nCarpeta: %s", emummc_img->part_sector[raw_btn_idx], &emummc_img->part_path[raw_btn_idx * 128]);
	...
	lv_label_set_static_text(label_txt3, "Archivo SD");
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_EDIT"  Administrar EmuNAND");
	...
	lv_obj_t *label_txt = lv_label_create(h1, NULL);
	lv_label_set_static_text(label_txt, "Info EmuNAND y Seleccion");
	lv_obj_set_style(label_txt, lv_theme_get_current()->label.prim);
	...
	lv_label_set_static_text(label_btn, emu_info.enabled ? "#96FF00 "SYMBOL_OK"  Activada!#" : "#FF8000 "SYMBOL_CLOSE"  Desactivada!#");
	...
	{
		if (emu_info.sector)
			s_printf(txt_buf, "#00DDFF Tipo:# Particion SD Raw\n#00DDFF Sector:# 0x%08X\n#00DDFF Carpeta Nintendo:# %s",
				emu_info.sector, emu_info.nintendo_path ? emu_info.nintendo_path : "");
		else
			s_printf(txt_buf, "#00DDFF Tipo:# Archivo SD\n#00DDFF Basado en Carpeta:# %s\n#00DDFF Carpeta Nintendo:# %s",
	...
	lv_label_set_static_text(label_btn, SYMBOL_SETTINGS"  Cambiar EmuNAND");
	...
	lv_label_set_static_text(label_txt2,
		"Elije entre imagenes creadas en la carpeta emuMMC o en\n"
		"particiones en la SD. Puedes tener maximo 3 basadas en\n"
		"particiones e infinitas basadas en archivo.");
	...
	lv_obj_t *label_txt3 = lv_label_create(h2, NULL);
	lv_label_set_static_text(label_txt3, "Herramientas de EmuNAND");
	lv_obj_set_style(label_txt3, lv_theme_get_current()->label.prim);
	...
	lv_label_set_static_text(label_btn, SYMBOL_DRIVE"  Crear EmuNAND");
	...
	lv_label_set_static_text(label_txt4,
		"Permite crear una EmuNAND en #C7EA46 archivo SD# o en #C7EA46 particion\n"
		"SD Raw. Puedes crearla desde la eMMC o desde un Backup.");
	...
	lv_label_set_static_text(label_btn, SYMBOL_SHUFFLE"  Migrar EmuNAND");
	...
	lv_label_set_recolor(label_txt4, true);
	lv_label_set_static_text(label_txt4,
		"Migra un backup a #C7EA46 Archivo SD# o repara una #C7EA46 Particion SD RAW#\n"
		"existente. Adicionalmente puedes migrar desde otras EmuNANDs.");
	lv_obj_set_style(label_txt4, &hint_small_style);
